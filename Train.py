...
def parse_args():
    ...
    parser.add_argument('--trainset_dir', type=str, default='./Datasets/Win5_FABLFQA_5x64x64/')
...
def train(train_loader, cfg, test_scene_id):
    ...
    for idx_epoch in range(0, cfg.n_epochs):
        ...
        for idx_iter, (data, score_label, cls_label) in tqdm(enumerate(train_loader), total=len(train_loader), leave=False):
            data, score_label, cls_label = Variable(data).to(cfg.device), Variable(score_label).to(cfg.device), Variable(cls_label).to(cfg.device)
            score_label = score_label.view(score_label.size()[0], -1)
            cls_label = cls_label.view(cls_label.size()[0])
            data = torch.unsqueeze(data, dim=1)
            score_out, cls_out = net(data)
            loss_main = torch.nn.MSELoss().to(cfg.device)(score_out, score_label)
            loss_aux = torch.nn.CrossEntropyLoss().to(cfg.device)(cls_out, cls_label)
        ...
        save_ckpt({'epoch': idx_epoch + 1, 'state_dict': net.state_dict(), 'loss': loss_list, },
                  ...
                  filename='FABLFQA_epoch' + str(idx_epoch + 1) + '.pth.tar')
        load_model_path = './log/' + str(test_scene_id[0]) + '_' + str(test_scene_id[1]) + \
                          '/FABLFQA_epoch' + str(idx_epoch + 1) + '.pth.tar'
